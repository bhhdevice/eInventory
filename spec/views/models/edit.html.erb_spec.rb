...
RSpec.describe "models/edit", type: :view do
  before(:each) do
    @model = assign(:model, create(:model))
  end
  ...
  it "renders the edit model form" do
    ...
    assert_select "form[action=?][method=?]", model_path(@model), "post" do
      ...
      assert_select "input#model_wifi_true[name=?]", "model[wifi]"
      assert_select "input#model_bluetooth_true[name=?]", "model[bluetooth]"
      assert_select "input#model_gps_true[name=?]", "model[gps]"
